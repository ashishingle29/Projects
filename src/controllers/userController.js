...
const createUser = async function (req, res) {
    ...
    let { fname, lname, email, phone,  password, address } = data; //Destructuring
      ...
      if (file && file.length == 0)
        return res.status(400).send({ status: false, message: "ProfileImage is a mandatory field" });
      ...
      if (findPhone)
        return res.status(400).send({status: false,message: "user with this phone number already exists"});
  
      if (!password)
        return res.status(400).send({ status: false, message: "password is mandatory" });
      if (!password.length >=8 && password.length <=15)
        return res.status(400).send({ status: false, message: "password is invalid (Password Should be Min 8 or Max 15 in length)" });
   //..hashing
            const saltRounds = 10;
...
const userUpdate = async function (req, res) {
  try {
    ...
    const { address, fname, lname, email, profileImage, phone, password } = data;

    const userdata = await userModel.findByIdAndUpdate({ userId: userId },...data
    );
